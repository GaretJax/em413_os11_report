...
\subsection*{Original System Problem Statement}
\textbf{To} Provide Persistent, low-latency access to space-based information
\textbf{By} optimizing the data transmission path to circumvent the geographic and orbital constraints 
\textbf{Using} a proliferated Low Earth Orbit (pLEO) constellation architecture with enhanced communication throughput capabilities.

\subsection*{Refined System Problem Statement}
\textbf{To} enable global decision making through persistent, low-latency space-based data \textbf{By} dynamically routing data through a networked constellation to bypass terrestrial infrastructure limitations 
\textbf{Using} a networked pLEO constellation architecture.

\subsection*{Discussion and Rationale for Scope}
The final SPS is the result of multiple iterations designed to avoid being too generic to offer direction or too solution-specific to allow for architectural discovery. 

\begin{itemize}
    \item \textbf{Intent (To):} The initial focus on ``delivering data'' was recognized as a functional process rather than a solution-neutral intent. By shifting to ``enable global decision making,'' the statement focuses on the value provided to the primary beneficiary. The specific inclusion of ``persistent, low-latency'' data provides a meaningful search direction that differentiates this architecture from traditional GEO-based systems.
    
    \item \textbf{Function (By):} We moved away from the term ``optimizing,'' which functioned as a vague verb lacking descriptive power. By choosing ``dynamically routing,'' we defined a functional transformation that is solution-neutral enough to allow for the exploration of various networking protocols while being specific enough to incorporate inter-satellite links (ISL).
    
    \item \textbf{Form (Using):} The scope was intentionally narrowed to a ``pLEO constellation architecture.'' While the sponsor (Airbus US) has broad interests in space systems, our team chose to narrow the scope to pLEO to provide the technical constraints required for meaningful architectural analysis. To maintain breadth within this form, we avoided overly detailed specifications like ``enhanced communication throughput,'' which we identified as a requirement-level detail that would prematurely mask solution-specific ideas.
\end{itemize}